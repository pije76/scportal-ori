...
class PowerMeasurementPoint(MeasurementPoint):
    """
    A C{PowerMeasurementPoint} is a L{MeasurementPoint} that measures
    power.
    """
    class Meta:
        proxy = True
        verbose_name = _('Power measurement point')
        verbose_name_plural = _('Power measurement points')
        app_label = 'customers'

    def __init__(self, *args, **kwargs):
        super(PowerMeasurementPoint, self).__init__(*args, **kwargs)
        if self.utility_type is None:
            self.utility_type = utilitytypes.OPTIONAL_METER_CHOICES.electricity

        if self.role is None:
            self.role = self.MEASUREMENT_POINT_POWER

    def save(self, *args, **kwargs):
        super(PowerMeasurementPoint, self).save(*args, **kwargs)
    ...
    def get_gauge_data_series(self):
        """
        C{PowerMeasurementPoint} implementation of
