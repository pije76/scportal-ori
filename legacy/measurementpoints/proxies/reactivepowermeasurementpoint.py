...
class ReactivePowerMeasurementPoint(MeasurementPoint):
    """
    A C{ReactivePowerMeasurementPoint} is a L{MeasurementPoint} that measures
    reactive power.
    """
    class Meta:
        proxy = True
        verbose_name = _('Reactive power measurement point')
        verbose_name_plural = _('Reactive power measurement points')
        app_label = 'customers'

    def __init__(self, *args, **kwargs):
        super(ReactivePowerMeasurementPoint, self).__init__(*args, **kwargs)
        if self.utility_type is None:
            self.utility_type = utilitytypes.OPTIONAL_METER_CHOICES.electricity

        if self.role is None:
            self.role = self.MEASUREMENT_POINT_REACTIVE_POWER

    def save(self, *args, **kwargs):
        super(ReactivePowerMeasurementPoint, self).save(*args, **kwargs)
        assert self.input_configuration

        graph, created = Graph.objects.get_or_create(
            collection=self,
            role=DataRoleField.REACTIVE_POWER)
        if created:
            Link.objects.create(
    ...
    def get_gauge_data_series(self):
        """
        C{ReactivePowerMeasurementPoint} implementation of
