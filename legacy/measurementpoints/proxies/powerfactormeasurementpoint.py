...
class PowerFactorMeasurementPoint(MeasurementPoint):
    """
    A C{PowerFactorMeasurementPoint} is a L{MeasurementPoint} that measures
    power factor.
    """
    class Meta:
        proxy = True
        verbose_name = _('Power factor measurement point')
        verbose_name_plural = _('Power factor measurement points')
        app_label = 'customers'

    def __init__(self, *args, **kwargs):
        super(PowerFactorMeasurementPoint, self).__init__(*args, **kwargs)
        if self.utility_type is None:
            self.utility_type = utilitytypes.OPTIONAL_METER_CHOICES.electricity

        if self.role is None:
            self.role = self.MEASUREMENT_POINT_POWER_FACTOR

    def save(self, *args, **kwargs):
        super(PowerFactorMeasurementPoint, self).save(*args, **kwargs)
        assert self.input_configuration

        graph, created = Graph.objects.get_or_create(
            collection=self,
            role=DataRoleField.POWER_FACTOR)
        if created:
            Link.objects.create(
    ...
    def get_gauge_data_series(self):
        """
        C{PowerFactorMeasurementPoint} implementation of
