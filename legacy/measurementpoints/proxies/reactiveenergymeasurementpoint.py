...
class ReactiveEnergyMeasurementPoint(MeasurementPoint):
    """
    A C{ReactiveEnergyMeasurementPoint} is a L{MeasurementPoint} that measures
    reactive energy.
    """
    class Meta:
        proxy = True
        verbose_name = _('Reactive energy measurement point')
        verbose_name_plural = _('Reactive energy measurement points')
        app_label = 'customers'

    def __init__(self, *args, **kwargs):
        super(ReactiveEnergyMeasurementPoint, self).__init__(*args, **kwargs)
        if self.utility_type is None:
            self.utility_type = utilitytypes.OPTIONAL_METER_CHOICES.electricity

        if self.role is None:
            self.role = self.MEASUREMENT_POINT_REACTIVE_ENERGY

    def save(self, *args, **kwargs):
        super(ReactiveEnergyMeasurementPoint, self).save(*args, **kwargs)
        assert self.input_configuration

        graph, created = Graph.objects.get_or_create(
            collection=self,
            role=DataRoleField.REACTIVE_ENERGY)
        if created:
            Link.objects.create(
    ...
    def _get_input_configuration(self):
        ...
        if self.id and not self._input_configuration:
            self._input_configuration = \
                ConsumptionAccumulationAdapter.objects.get(
    ...
    def get_gauge_data_series(self):
        """
        C{ReactiveEnergyMeasurementPoint} implementation of
    ...
    def get_input_configuration_choices():
        """
        Get L{ConsumptionAccumulationAdapter} choices.
        """
