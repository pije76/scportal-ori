//
// EmonTx v3.4 Continuous sampling With WH Totals Beta
//
...
{ 
  ...
  if (digitalRead(DIP_switch1)==LOW && digitalRead(DIP_switch2)==LOW) nodeID = 4;                            // IF DIP switch 1 is switched on then subtract 1 from nodeID
  if (digitalRead(DIP_switch1)==HIGH && digitalRead(DIP_switch2)==LOW) nodeID = 5;
  if (digitalRead(DIP_switch1)==LOW && digitalRead(DIP_switch2)==HIGH) nodeID = 6;
  if (digitalRead(DIP_switch1)==HIGH && digitalRead(DIP_switch2)==HIGH) nodeID = 7;
...
void loop()
{
  if (EmonLibCM_Ready())   
  {
    ...
    if (EmonLibCM_ACAC) {
      ...
      emontx.kw1 = EmonLibCM_getWattHour(0);
      emontx.kw2 = EmonLibCM_getWattHour(1);
      emontx.kw3 = EmonLibCM_getWattHour(2);
      emontx.kw4 = EmonLibCM_getWattHour(3);
    } else {
